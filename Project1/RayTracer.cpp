...
bool RayTracer::rayTriangleIntersect(const Vec3& rayDir, const Triangle triangle, float& t, Vec3& P, Vec3 startPos) {
	...
	q = vecCross(s, edge1);
	vT = 1.0 / a * vecDot(rayDir, q);

	if (vT < 0.0 || uT + vT > 1.0)
		return false;

	float curr_t = 1.0 / a * vecDot(edge2, q);
...
bool RayTracer::isVisible(Vec3 lightDir, Vec3 oldP, float timeTolightSource) {
	t = infty;
	Vec3 dir = lightDir;
	IntersectBVHNode(dir, bvh->bvhNode, 0, oldP);
	float curr_t = t;
	if (t < timeTolightSource) {
		return 0;
	}
	return 1;
}
...
Vec3 reflectedRayDir(Vec3 rayDir, Vec3 normal) {
	Vec3 reflectedRay;
	reflectedRay = normal * vecDot(rayDir*(-1.0f), normal) * 2 + rayDir;
	return reflectedRay;
}
...
Vec3 RayTracer::computeColor(Vec3 rayDir, Vec3 mynormal, const Vec3 mydiffuse, const Vec3 myspecular, const float myshininess, const Vec3 myambient, const Vec3 myemission) {
	...
	for (int i = 0; i < directional.size(); i += 6) {
		...
		bool visible = isVisible(lightDir, oldP + lightDir * EPSILON, infty);
		if (visible) {
	...
	for (int i = 0; i < point.size(); i += 6) {
		...
		halfVec = vecNormalize(lightDir - rayDir);
		float timeToLS = vecLength(lightPos - oldP);
		bool visible = isVisible(lightDir, oldP + lightDir * EPSILON, timeToLS);
		if (visible) {
			float distance = sqrt((lightPos - oldP).x * (lightPos - oldP).x + (lightPos - oldP).y * (lightPos - oldP).y + (lightPos - oldP).z * (lightPos - oldP).z);
			float atten = 1.0 / ((attenuation.z * distance * distance) + (attenuation.y * distance) + attenuation.x);
			color = color + computeLight(lightDir, lightColor, oldnormal, halfVec, mydiffuse, myspecular, myshininess)*atten;
